...
- **extrends one-time cleanup (HIGH, BLOCKING extrends onboarding).** extrends' STATUS.md is currently 449 lines / ~27k tokens with 38 change headers — the cap rule is now in the scaffold, but extrends' existing backlog must be cleaned up manually: cap its STATUS.md to the last 3 paragraphs, move ~35 old entries to `ai-docs/archive/status-log.md`, and sweep resolved open-questions to `ai-docs/archive/retired-notes.md`. This is **per-repo state** and cannot sync via `sync_scaffold.py` — it must be done directly in extrends. Until this cleanup happens, new extrends agent sessions will still hit the Read cap on the MANDATORY-block files even though the corrected rule has propagated.
