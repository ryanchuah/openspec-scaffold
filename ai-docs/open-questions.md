...
- **[NEW open question] `open-questions.md` is not actually bounded by the retired-notes mechanic in a high-activity repo (raised 2026-06-17).** cap-status-log assumed `open-questions.md` could stay "read in full" because resolved items prune to `retired-notes.md`. But extrends' `open-questions.md` remained **over the Read tool's length cap even after** the conservative resolved-items prune above — the file is dominated by *genuinely-open* follow-ons (one dense bullet per shipped change, accreting faster than items resolve), so the prune mechanic alone cannot bound it. Candidate scaffold follow-ups: (a) give `open-questions.md` the same AGENTS.md read-directive treatment decisions.md got (header-scan `## ` sections, then read only the relevant ones) — cheapest, no structural change; or (b) restructure into a small always-loaded "active/blocking" file + an on-demand parked-followups file. Pairs with the `cap-status-log` rationale in `decisions.md`. Severity 🟡 (onboarding-cost, not correctness). Not yet scoped into a change.

- **psc-monitor — no action.** psc-monitor is still small (STATUS.md well under the cap) and inherits the corrected rule on the next `sync_scaffold.py` run.
