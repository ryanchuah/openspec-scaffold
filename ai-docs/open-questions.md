...
Convergence-guard correctness fixes A1–A5: oscillation detection + absolute backstop ceiling, test-scoped signature + basename-reduced state key, deterministic git-diff-derived file-touch detection with `--editing` demoted to optional hint, path-shaped regex replacing greedy `/\S+`, and `test-gate.sh` cwd-fix. Full decision in `ai-docs/decisions.md` (existing "Apply-executor convergence detection" entry updated in-place for A3). Archive: `openspec/changes/archive/2026-06-17-fix-convergence-guard`. Modified spec: `openspec/specs/apply-convergence-guard/spec.md`. Modified agents: `.opencode/agents/apply-executor.md`, `.claude/agents/apply-executor.md`.
...
STATUS.md is now capped at 3 change paragraphs, with overflow moved verbatim to `ai-docs/archive/status-log.md` during archive reconciliation. AGENTS.md's read directive for decisions.md was changed to header-scan + relevant entries. Both archive-executor bodies carry the prune mechanic and are byte-identical. No delta specs — MEDIUM change, tasks.md only. Full decision in `ai-docs/decisions.md` ("cap-status-log — STATUS.md cap rule, decisions.md read-directive change"). Archive: `openspec/changes/archive/2026-06-17-cap-status-log`.

- **extrends one-time cleanup (IN PROGRESS).** Per-repo state — done directly in extrends, not via `sync_scaffold.py`.
  - *STATUS.md cap — DONE (extrends commits `7a2a49e`, `f642541`, local/unpushed).* Capped 38→3 change paragraphs; 35 moved verbatim to `extrends/ai-docs/archive/status-log.md` (newest-first); findability pointers added (`## Pointers` + inline note); byte-integrity + findability verified across two deepseek-pro information-loss review rounds (final VERDICT READY, zero defects). Migration script: `extrends/scripts/_status_cap_oneoff.py`.
  - *Propagate the cap rule to extrends — REMAINING (operator-approved).* Run `python3 scripts/sync_scaffold.py ../extrends` so extrends' AGENTS.md + archive-executor carry the cap rule; otherwise extrends' next archive uses the old append-only executor and re-bloats STATUS.md (and won't maintain `status-log.md`). Dry-run `--check` first; confirm only cap-status-log's 4 managed files DIFFER (no clobber of extrends-local edits).
  - *Prune extrends `open-questions.md` — REMAINING (operator-approved).* Still 794 lines / ~26k tokens, over the Read cap. Judgment-heavy: move only unambiguously resolved/shipped items to `ai-docs/archive/retired-notes.md`, NOT a bulk sweep; re-run the pro information-loss review after.

- **psc-monitor — no action.** psc-monitor is still small (STATUS.md 119 lines, no cap issue) and inherits the corrected rule on the next `sync_scaffold.py` run.
