...
- **Phase 2 propagation to extrends + psc-monitor — PENDING (HIGH).** This change edits scaffold-managed/manifest files; until `sync_scaffold.py` carries them downstream the three repos diverge. Needs operator in-session go-ahead for the cross-repo writes.
