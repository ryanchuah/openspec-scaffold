...
Open-questions.md horizon split: the always-loaded surface is now bounded to active items only (open blockers, operator-decision items, in-flight backlogs that gate other work); the deferred/monitored long tail lives in `ai-docs/parked-follow-ons.md`. Full decision and rationale in `ai-docs/decisions.md` ("split-open-questions (2026-06-17)"). Archive: `openspec/changes/archive/2026-06-17-split-open-questions`.

- **Propagation backlog (HIGH, BLOCKING downstream effect).** This change's four managed files (AGENTS.md, both archive-executor bodies, `.claude/skills/openspec-archive-change/SKILL.md`) are scaffold-only and inert in extrends + psc-monitor until a `sync_scaffold.py` run carries them downstream. After archive: run `sync_scaffold.py --check` (dry-run) against each downstream repo, confirm only the managed files DIFFER, apply + re-check IDENTICAL, review each repo's AGENTS.md span-merge (title / `## Project context` / tail preserved), commit per repo. Needs explicit operator in-session go-ahead for the cross-repo writes; do NOT push.
