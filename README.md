...
| `knowledge/STATUS.md` | Live project status — what's done, what's next |
| `knowledge/decisions/INDEX.md` | Durable architectural decisions and rationale |
...
| `.opencode/agents/openspec-verifier.md` | DeepSeek V4 Flash verifier agent — runs independent multi-model verification passes during verify (read diffs, re-run suite, eyeball output); read-only on files; invoked via `opencode run --agent openspec-verifier` on both platforms |
...
**`knowledge/STATUS.md`** — update the "Immediate next action" line.
...
| "verify \<name\>" | The orchestrator's own behavioral review — re-run suite, eyeball real output, re-delegate fixes — followed by independent multi-model verification passes (MEDIUM: a pro behavioral pass; COMPLEX: adds a flash lens pass; identical on both platforms) as hard gates before the checklist |
...
| DeepSeek V4 Flash (`openspec-verifier`, project-local) | Verifier — runs independent multi-model verification passes during verify (read diffs, re-run suite, eyeball output); read-only on files; invoked via `opencode run --agent openspec-verifier` (called automatically after the orchestrator's self-review) |
...
| DeepSeek V4 Pro (via `opencode run`) | verifier — behavioral verification pass (pro tier) during verify (MEDIUM and COMPLEX); invoked automatically after the orchestrator's self-review |
| DeepSeek V4 Flash (via `opencode run`) | verifier — lens verification pass (flash tier, COMPLEX only) during verify — a test-quality or data-scale lens, not a repeat of the behavioral checklist; runs after the pro behavioral pass |
...
- **Write discipline:** during a change, write its `openspec/changes/<name>/` files freely (check off `tasks.md`, jot `notes.md`). Do **not** edit `knowledge/STATUS.md` / `knowledge/` mid-change — that keeps the working context small.
- **Archive = handoff:** archive is where the project docs in `knowledge/` get reconciled from the change dir. Delegated to a `deepseek/deepseek-v4-pro` archive-executor (fresh context; Claude via `opencode run`, OpenCode via a subagent), then reviewed and committed by the primary — that keeps the multi-file reconciliation cheap.
