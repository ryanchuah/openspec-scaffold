...
`PREMISE: AGREE`. A `PREMISE: DISSENT` SHALL block the freeze regardless of severity count. The propose
workflow SHALL assert the reviewer output for `proposal.md` contains a `PREMISE: AGREE` or
`PREMISE: DISSENT` line; a missing premise line SHALL be treated as a failed pass (do not freeze, re-run),
not as implicit agreement. `design.md` and `tasks.md` SHALL retain the existing zero-🔴 freeze rule.

#### Scenario: Clean but dissented proposal does not freeze
- **WHEN** a `proposal.md` review returns zero 🔴 but `PREMISE: DISSENT`
- **THEN** the proposal does not freeze and the dissent is surfaced for a direction decision

#### Scenario: Missing premise line is a failed pass
- **WHEN** a `proposal.md` review returns zero 🔴 but omits any `PREMISE:` line
- **THEN** the propose workflow treats it as a failed pass and re-runs rather than freezing

#### Scenario: Clean and agreed proposal freezes
- **WHEN** a `proposal.md` review returns zero 🔴 and `PREMISE: AGREE`
- **THEN** the proposal freezes and the workflow proceeds to the next artifact
