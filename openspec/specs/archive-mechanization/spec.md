...
Make the deterministic parts of the archive phase — the change-directory move and the promotion of
ADDED/REMOVED/RENAMED spec-delta operations into canonical main specs — deterministic scripts rather
than LLM prose, so the LLM is reserved for the operations that need judgment (MODIFIED merges and doc
reconciliation) and canonical specs are never corrupted by a stochastic editor.
