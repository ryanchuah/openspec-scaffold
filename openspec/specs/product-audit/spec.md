...
Define the protocol contract for the promise-surface / business-thesis audit — the seventh,
copy-and-thesis-facing audit class (object = the product's pricing/landing/marketing copy and business
thesis; oracle = the code and the market), the inverse of the code-facing audit classes. The protocol is
owned by the scaffold as a single operator-invoked, pull-only, single-session skill that standardizes the
blind attack-list → five-lane evidence fan-out → per-attack disposition diff → operator ratification menu
→ machine-discriminable `PRODUCT:` verdict sequence; the claims-ledger convention (every externally
visible promise → delivering code surface → proving check) guarded by a content-hash staleness lint;
source-class labeling and the entitlement-reachability / severity-taxonomy-completeness prompts; and
close-out routing into the finding-closure ratchet using its existing dispositions with no new close-out
machinery. Being single-session, it carries no in-progress liveness obligation.
