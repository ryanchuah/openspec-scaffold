...
Define one shared definition of "green" for every scaffolded repo — a scaffold-managed `ruff.toml`
(selects E, F, I, B rules + enforced format; E501 ignored), a single `scripts/check.sh` gate (ruff
check → `ruff format --check` → per-repo `scripts/test-cmd`) that the commit hook, CI, and humans
all invoke, doc-lints (`knowledge_lint`/`status_lint`) enforced on the live tree via pytest, and
documented+provisioned security scanners (`gitleaks`, `osv-scanner`) with a `go install` local-dev
helper.
