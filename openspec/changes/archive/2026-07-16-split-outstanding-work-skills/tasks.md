...
- [x] 1.1 `git mv .claude/skills/outstanding-work-review .claude/skills/outstanding-work-scan`.
- [x] 1.2 In `.claude/skills/outstanding-work-scan/SKILL.md` frontmatter: set `name: outstanding-work-scan`
      ...
      skill by slug).
- [x] 1.3 Narrow the skill body to the cheap path. Under **Steps → 3. Judge (orchestrator)**: REMOVE
...
- [x] 1.4 Update any body prose in the scan SKILL.md that self-references the old name
...
- [x] 2.1 Create `.claude/skills/outstanding-work-deep-sweep/SKILL.md` with frontmatter mirroring
...
- [x] 2.2 Body Step 1 — **Run the deterministic scan first:** invoke the `outstanding-work-scan`
      skill (its gather + read + verify + untriaged-bucket dedup) and consume its snapshot before any
      residual sweep.
- [x] 2.3 Body Step 2 — **Five-category residual sweep as parallel subagents.** Condense the five
...
- [x] 2.4 Body Step 3 — **Triage into trackers:** promote genuinely uncaptured items into
...
- [x] 2.5 Add a **Guardrails** block mirroring `correctness-audit`: operator-invoked, pull-only; NEVER
...
- [x] 3.1 In `scripts/scaffold_manifest.txt`, repoint the existing
...
- [x] 3.2 In `scripts/scaffold_manifest.txt`, ADD a new line
      `.claude/skills/outstanding-work-deep-sweep/SKILL.md` in the correct ordered position.
- [x] 3.3 In `scripts/scaffold_manifest_removed.txt`, append
...
- [x] 4.1 In `AGENTS.md`, "Working process" section, rename the pull-only bullet reference
...
- [x] 5.1 `scripts/check.sh` runs green (ruff + format + pytest, incl. scaffold_lint invariant).
- [x] 5.2 `python3 scripts/scaffold_lint.py` clean.
- [x] 5.3 `openspec validate split-outstanding-work-skills --strict` exits 0.
- [x] 5.4 `grep -rn "outstanding-work-review" .` shows stale refs ONLY in the expected
      ...
      **Resolved (orchestrator):** the executor surfaced one extra ref —
      `openspec/changes/knowledge-lint-gitignored-citation-exempt/plan.md:53`, an "Out of scope"
      note in a *shipped SMALL change's* plan recording that the split was deferred to a separate
      change. It names the old skill because it describes the state at that time (period-correct,
      like research/archive dirs) and is another change's frozen artifact. **Allowlisted as a
      legitimate historical reference — deliberately NOT edited.** `check.sh` stayed green after the
      HANDOFF.md deletion, so no broken-citation fallout.
