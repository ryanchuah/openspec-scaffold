...
- [x] 1.1 Create `scripts/scaffold_lint.py`: stdlib-only CLI, `python3 scripts/scaffold_lint.py [--root PATH]`
...
- [x] 1.2 Implement check `manifest-completeness`: (a) every existing file matching the managed globs
...
- [x] 1.3 Implement check `agents-md-structure` as TWO sub-checks (the reused function only detects
...
- [x] 1.4 Implement check `config-rules-last`: `openspec/config.yaml` must contain a `rules:` block and
...
- [x] 1.5 Implement check `dangling-skill-refs`: scan `AGENTS.md` plus every `.md` under
...
- [x] 1.6 Implement check `budget-agreement`: extract every numeric `timeout -k <G> <B>` pair from the
...
- [x] 2.1 Create `scripts/test_scaffold_lint.py` (pytest style, `tmp_path` fixture repos, same
...
- [x] 2.2 Fix the one known live violation so the repo lints clean: in
...
- [x] 2.3 Add a live-repo test to `scripts/test_scaffold_lint.py`: running the linter against THIS
...
- [x] 3.1 In `scripts/sync_scaffold.py`, in BOTH `check()` and `sync()`: after the existing manifest
...
- [x] 3.2 Add tests to `scripts/test_sync_scaffold.py`: target with a settings.json containing
...
- [x] 4.1 Create `scripts/test-cmd` containing exactly one line: `pytest -q`
...
- [x] 4.2 Final convergence check across everything this change touched (groups 1–3): run
