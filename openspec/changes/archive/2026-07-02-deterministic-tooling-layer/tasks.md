...
- [x] 1.1 Create `scripts/audit_scope.py` with three subcommands. `scan` (default, read-only):
  ...
  subprocess failure.
- [x] 1.2 `tag --date YYYY-MM-DD` (date required — no implicit "today", keeping runs reproducible):
...
- [x] 1.3 `log-line --date YYYY-MM-DD --essence "<free text>"`: PRINT (never write) the audit-log
...
- [x] 1.4 Create `scripts/test_audit_scope.py` (stdlib unittest). Build a real temp git repo
...
- [x] 2.1 Create `scripts/data_lint.py`. CLI: `--checks-dir <dir>` (default `checks/`), `--db-url`
...
- [x] 2.2 Output + exit contract: JSON `{generated_by, checks: [{name, status: pass|fail, rows,
...
- [x] 2.3 Create `scripts/test_data_lint.py` (stdlib unittest, stub `psql` shell script on PATH that
...
- [x] 3.1 Create `scripts/index_coverage.py`. Inputs: `--schema <path-or-glob>` (DDL text: concatenated
...
- [x] 3.2 Output contract: JSON `{generated_by, "confidence": "lead", leads: [{table, column, usage:
...
- [x] 3.3 Create `scripts/test_index_coverage.py` (stdlib unittest, fixture DDL + query strings written
...
- [x] 4.1 Create `scripts/audit_bundle.py` skeleton + config loading. Config file: `audit.toml` at the
...
- [x] 4.2 Check registry. Built-in parsed checks (native JSON output → normalized findings
...
- [x] 4.3 Availability probes + pinning. Module constant `EXPECTED_TOOL_VERSIONS` pins **binary** tools
  ...
  the listing.
- [x] 4.4 Modes. `--list`: one line per registered check — `<name>  <tier>  <enabled|disabled>
  ...
  / the `--json` targets (D3).
- [x] 4.5 Output contract (D8b). Per check: `<out>/<check>.json` (or `.txt` for custom/unparsed) plus
...
- [x] 4.6 Baseline diff (D6/D8d). `--baseline <prior-findings.json>` — valid ONLY with `--report`
...
- [x] 4.7 Create `scripts/test_audit_bundle.py` (stdlib unittest; stub executables on PATH emitting
...
- [x] 5.1 In `AGENTS.md` (golden copy, inside the scaffold-managed shared span — i.e. anywhere before
...
- [x] 5.2 In `knowledge/README.md` (scaffold-managed), add `knowledge/audit-log.md` to the taxonomy as
...
- [x] 5.3 In `scripts/scaffold_manifest.txt` under `# Scripts`, add eight lines — `scripts/audit_bundle.py`,
...
- [x] 5.4 Run the full scaffold suite from the repo root and confirm green: every `scripts/test_*.py`
