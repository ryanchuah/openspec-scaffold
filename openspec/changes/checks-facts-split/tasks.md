...
- [x] 1.1 `git mv scripts/audit_bundle.py scripts/checks.py`. Update the module docstring
...
- [x] 1.2 Add a `family` field to every `_REGISTRY` entry: `"check"` for ruff, gitleaks,
...
- [x] 1.3 In `scripts/checks.py`, scope the run modes by family: `--floor` selects
...
- [x] 2.1 Add per-check metadata to `_REGISTRY` entries that need an external binary:
...
- [x] 2.2 Preflight in `_mode_multi` (`--floor`/`--report`): BEFORE executing any check,
...
- [x] 2.3 Keep stop-on-first-failure for infra failures that arise mid-run (a binary that
...
- [x] 2.4 `--list`: after the listing, when any enabled check-family entry is
...
- [x] 3.1 Create `scripts/facts.py`: a thin CLI importing the engine from
...
- [x] 3.2 Inventory gains an `audit_anchor` field: `{"tag": <latest audit/* tag or
...
- [x] 4.1 `_load_config` in checks.py: read `checks.toml` instead of `audit.toml`
      (config-source label `"checks.toml"|"defaults"`). No fallback read of
      `audit.toml` — no repo has one.
- [x] 4.2 `scripts/knowledge_lint.py`: the config read at line 290
      ...
      `audit.toml`).
- [x] 4.3 `--report` output dir in checks.py: `output/audit/<date>/` →
      `output/checks/<date>/` (the literal at ~1304). `--floor`/`--check` CWD default
      unchanged.

## 5. Manifest, removed-paths, and test-file renames

- [x] 5.1 `git mv scripts/test_audit_bundle.py scripts/test_checks.py`; update imports
      ...
      fixtures.
- [x] 5.2 `scripts/scaffold_manifest.txt`: replace `scripts/audit_bundle.py` and
...
- [x] 5.3 `scripts/scaffold_manifest_removed.txt` (exists from the sync-deletion-manifest
...
- [x] 6.1 Rework `MissingBinaryAbortTest`/`ResumeOrderProofTest` (now in test_checks.py)
...
- [x] 6.2 Add preflight-message shape tests: the INFRA-FAIL line carries trigger,
...
- [x] 6.3 New `scripts/test_facts.py` — and in the same task add
...
- [x] 6.4 `audit_scope.py` ceremony updates: `cmd_log_line` prints the stderr hint —
...
- [x] 7.3 AGENTS.md "Deterministic audit tooling" section: rewrite to the
...
- [x] 7.4 Write the delta spec `openspec/changes/checks-facts-split/specs/knowledge-lint/spec.md`
...
- [x] 7.5 Repoint stragglers — this exact live-reference list (verified 2026-07-03),
