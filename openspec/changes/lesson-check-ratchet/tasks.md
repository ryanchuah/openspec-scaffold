...
- [ ] 1.1 Create `scripts/repo_lint.py` (stdlib-only): discover checks via flat sorted
...
- [ ] 1.2 Create `scripts/test_repo_lint.py` (fixture-based, tmp_path): clean check →
...
- [ ] 2.1 Register `repo-lint` in `scripts/checks.py`: registry entry (tier `floor`,
...
- [ ] 2.2 Extend `scripts/test_checks.py` (note: this file uses `unittest` + fake shell
...
- [ ] 3.1 Add `_check_ratchet_log(root)` to `scripts/knowledge_lint.py`, guarded on
...
- [ ] 3.2 Extend `scripts/test_knowledge_lint.py` with fixture cases: absent ledger
...
- [ ] 4.1 Create `knowledge/ratchet-log.md` with a header comment (HTML comment or plain
...
- [ ] 4.2 Add `knowledge/ratchet-log.md` to the Reference taxonomy row of
...
- [ ] 4.3 Add one bullet to `AGENTS.md` `## Working process` (inside the synced span):
...
- [ ] 4.4 Add `scripts/repo_lint.py` and `scripts/test_repo_lint.py` to
...
- [ ] 5.1 `.claude/skills/openspec-archive-change/SKILL.md`, Step 6 (primary review):
...
- [ ] 5.2 `.claude/skills/run-audit/SKILL.md`, Step 3 (Triage): insert the same
...
- [ ] 5.3 `.claude/skills/knowledge-drift-review/SKILL.md`, Step 2 (the stale-claims
...
- [ ] 6.1 Run `scripts/check.sh` (ruff + format + full pytest incl. live-tree lint gate
