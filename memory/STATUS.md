...
Instruction-surface rule-restatements single-sourced — the C2/W7 audit finding that closed the consolidation-plan ledger. Five rule-families (tasks.md = apply-only, model-assignment matrix, never-record-counts, mock-API war-story, web-research convention) were assigned one canonical home each; every other site now carries its per-context specifics + a citation, with the full rule preserved verbatim at its home. The self-enforcing convention and a five-row single-source registry live in `memory/lessons.md` §2. No behavior change — all `--model` invocation lines and `model:` frontmatter fields preserved byte-for-byte; `scripts/test_executor_body_agreement.py` stays green. MEDIUM change (tasks.md only), mirrors the `dedup-scaffold` extraction-not-redesign pattern. Verify: self-review + deepseek-v4-pro + deepseek-v4-flash verifier passes all READY, zero defects; guards and `openspec validate --all` green. The Phase 2 propagation (re-sync changed managed files to extrends + psc-monitor via `sync_scaffold.py`) is pending operator go-ahead — tracked in `memory/questions/INDEX.md`. Rationale in `memory/decisions/INDEX.md`; parked follow-on (config.yaml rules drift risk across repos) in the Parked section of `memory/questions/INDEX.md`. Archive: `openspec/changes/archive/2026-06-17-single-source-rules`.


## Immediate next action
No proactive build in flight. `add-status-lint` shipped and committed locally (unpushed). The linter
now mechanically gates `memory/STATUS.md`/`memory/decisions/INDEX.md` bounds at archive time — watch the next real archive to
