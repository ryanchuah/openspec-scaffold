...
- Upstream opencode empty-completion-as-clean-exit-0 (detect-truncated-stream follow-on, shipped 2026-07-18): consider upstream issue report / version pin → `knowledge/questions/opencode-empty-completion-exit-zero.md`
- Flash verifier non-English verdict block breaks English `--require-marker` asserts (detect-truncated-stream follow-on, shipped 2026-07-18): observed this change; adjacent to parked `extract_text` last-part-only defect → `knowledge/questions/flash-verifier-non-english-verdict-block.md`
