...
audit layer** — no `audit.toml`, no `checks/` directory, no task-runner `audit-*` targets, no
seeded `knowledge/audit-log.md`. The skill's commands and error-handling are verified accurate
(manual review of the SKILL.md against the actual `audit_bundle.py`/`audit_scope.py` CLI), but
...
- Its wiring-detection branch explicitly handles the missing-layer case (detects absent
  `audit.toml`/`checks/`/task targets and guides the build-out rather than failing opaquely).
