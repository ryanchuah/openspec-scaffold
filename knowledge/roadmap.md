...
**Update 2026-07-13:** OW-2 (`lesson-check-ratchet`) and OW-3 (`verify-stack-redirect`) are **SHIPPED** (`openspec/changes/archive/2026-07-13-lesson-check-ratchet/`, `openspec/changes/archive/2026-07-13-verify-stack-redirect/`) — OW-3 delivered the "redirect the third pass to a lens" token move this backlog names. Remaining frozen batch OW-5 → OW-6 applies in that hard order (see `knowledge/STATUS.md` Immediate next action); this backlog entry is reconciled/closed when that batch fully archives. OW-1/OW-4/OW-7–14 remain un-proposed.
