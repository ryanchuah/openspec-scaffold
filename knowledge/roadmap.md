...
## Context onboarding — lean AGENTS.md + bound the mutable state files — ✅ DONE 2026-06-18 — COMPLETE
**Priority:** ~~Medium~~ — COMPLETE, no action.
**Resolution:** All three levers resolved: lever 1A (psc-monitor appendix relocation to on-demand reference files) shipped via `lean-boot-context`; lever 1B (state-file bounding — cap STATUS, headers-only decisions, archive resolved entries) shipped via `add-status-lint`; lever 1C (bootstrap_context.py compiler) ruled out. Full analysis: `knowledge/research/explore-agent-context-infra-2026-06-18.md`.
